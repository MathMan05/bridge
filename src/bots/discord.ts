...
class Discord implements Bot {
	...
	constructor(info: {token: string; name: string}, helper: Helper) {
	...
	async connect() {
		return new Promise<void>(async (res) => {
			...
			this.client.on("messageDelete", (message) => {
				this.messageDConvert(message.id, message.channel.id);
			});
	...
	async messageDConvert(messageId: string, channelId: string) {
